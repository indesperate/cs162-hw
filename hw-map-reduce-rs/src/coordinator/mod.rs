...
use crate::{JobId, TaskNumber, WorkerId, COORDINATOR_ADDR, INITIAL_JOB_ID, INITIAL_WORKER_ID};
        ...
        match job_state.job_process {
            JobProcess::Map => {
                if let Some(map_task) = job_state.get_map_job() {
                    log::info!("Send Map job to worker {}!!", worker_id);
            ...
            JobProcess::Reduce => {
                if let Some(reduce_task) = job_state.get_reduce_job() {
                    log::info!("Send reduce job to worker {} !!", worker_id);
